...
const CartPage = () => {
 ...
 const{cartDetails,CartCount,setCartCount}=useContext(Usercontext)
    ...
    const getCartItems=async(UserId)=>{
        ...
        const Items=await BaseUrl.post("/api/view_cart",{
            UserId,
    ...
    const GiveSingleProduct= async(ProductIdArray)=>{
        ProductIdArray?.map(async(id)=>{
            ...
            const singleProduct= await BaseUrl.post("/api/get-SingleProduct",{
    ...
    const QuantityChange=async(ProductId,type,Quantity)=>{
        ...
        const CartItems=await BaseUrl.post("/api/cartQuantity",{
            ProductId:ProductId,
            qty:qty
    ...
    const Delete=async(ProductId)=>{
        ...
        const RemoveItem=await BaseUrl.post("/api/remove-cart-item",{
            ProductId
        })
...
const checkout=async()=>{
    ...
    // const cartItems=await BaseUrl.post("/api/checkout",{
    //     UserId:UserId
    // })
...
useEffect(()=>{
  ...
  const sendMsg=async(data)=>{
    const ordermsg=await BaseUrl.post("/api/send_msg",{
        msg:data
  ...
  const handlePayment = async () => {
    try {
        const res = await BaseUrl.post("/api/payment_order", {
            amount:TotalPrice
...
const handlePaymentVerify = async (data) => {
    ...
    const options = {
        ...
        handler: async (response) => {
            ...
            try {
                const res = await BaseUrl.post("/api/verify_order", 
                    {
...
const getPaymentType=async(data)=>{
    ...
    const fetchpaymentData=await BaseUrl.post("/api/fetch-payment",{
        paymentId
    })
    ...
    const NewOrders=await BaseUrl.post("/api/create_orders",{
    ...
    const sendMail=await BaseUrl.post("/api/send_email",{
        orderDetails:OrderDetails
    })

    console.log("Mail",sendMail)

    const sendMsg=await BaseUrl.post("/api/send_msg",{
        msg:JSON.stringify(OrderDetails,null,2)
    })

    console.log(sendMsg.data)

    // const SendWhatsAppMsg = await Baseu.post("/api/send_msg",{
